...
function normalizeMsg(raw) {
  return {
    ...
    is_admin:
      raw.is_admin !== undefined ? Number(raw.is_admin) : raw.isAdmin ? 1 : 0,
...
export default function AdminChatPanel({ token }) {
  ...
  // Stable ref so connectWS closure never goes stale on fetchUsers
    ...
    async (userId) => {
      ...
      try {
        const { data } = await axios.get(
          `${API_URL}/api/admin/chats/${userId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const connectWS = useCallback(() => {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        // Use Number() on both sides — DB returns numeric IDs but WS payload
        // may deliver them as strings, causing === to silently never match.
        const fromUserId = Number(msg.userId);
        const isOpenConvo = Number(activeRef.current?.user_id) === fromUserId;
        ...
        setUsers((prev) => {
          ...
          // Use a ref so the WS closure never goes stale
          ...
          setMessages((prev) => {
            ...
            const optIdx = prev.findIndex(
              (m) =>
                ...
                Boolean(m.is_admin) === Boolean(msg.is_admin),
            );
            if (optIdx !== -1) {
              const next = [...prev];
              next[optIdx] = msg;
    ...
    ws.onerror = () => ws.close();
  }, [token]); // fetchUsers accessed via fetchUsersRef — no stale closure
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] bg-[#0a0a0d] rounded-2xl border border-white/10 overflow-hidden">
      {/* ── Left: user list ── */}
      <div
        className={`flex flex-col border-r border-white/10 transition-all ${
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {loadingMsgs ? (
              ...
              messages.map((msg) => (
                ...
                >
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-1 text-xs font-black ${
                      msg.is_admin
                  ...
                  <div
                    className={`max-w-[70%] flex flex-col ${msg.is_admin ? "items-end" : "items-start"}`}
                  >
                    <div
