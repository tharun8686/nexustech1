  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (msg.type === 'chat_message') {
        ...
        const senderName = client.username || (client.isAdmin ? 'Admin' : 'User');
        ...
        db.query(
          ...
          (err, result) => {
            if (err) { console.error('Chat DB error:', err.message); return; }
...
app.get('/api/admin/promo-codes', authenticateAdmin, async (req, res) => {
  try {
    const [rows] = await promiseDb.query('SELECT * FROM promo_codes ORDER BY promo_id DESC');
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.post('/api/admin/promo-codes', authenticateAdmin, async (req, res) => {
  const { code, description, discount_type, discount_value, max_discount, min_order_value, usage_limit, valid_from, valid_until, is_active } = req.body;
  ...
  try {
    const [r] = await promiseDb.query(
      `INSERT INTO promo_codes (code, description, discount_type, discount_value, max_discount, min_order_value, usage_limit, valid_from, valid_until, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        code.trim().toUpperCase(), description || null,
    ...
    res.status(201).json({ message: 'Promo code created!', promo_id: r.insertId });
  } catch (err) {
...
app.put('/api/admin/promo-codes/:id', authenticateAdmin, async (req, res) => {
  const { description, discount_type, discount_value, max_discount, min_order_value, usage_limit, valid_from, valid_until, is_active } = req.body;
  try {
    await promiseDb.query(
...
app.delete('/api/admin/promo-codes/:id', authenticateAdmin, async (req, res) => {
  try {
    await promiseDb.query('DELETE FROM promo_codes WHERE promo_id = ?', [req.params.id]);
    res.json({ message: 'Promo code deleted.' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.patch('/api/admin/promo-codes/:id/toggle', authenticateAdmin, async (req, res) => {
  try {
    await promiseDb.query(
      'UPDATE promo_codes SET is_active = NOT is_active WHERE promo_id = ?',
      [req.params.id]
    );
    const [[row]] = await promiseDb.query('SELECT is_active FROM promo_codes WHERE promo_id = ?', [req.params.id]);
...
app.post('/api/admin/chats/:userId/send', authenticateAdmin, async (req, res) => {
